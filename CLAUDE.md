...
Two further skills operate on the conversation itself rather than advancing either pipeline: `/discussion` (stable IDs and full-list repaints for multi-item threads) and `/disproof-review` (a review contract where severity carries evidence like any other claim; user-invoked only).
...
- **`/disproof-review`** — Review contract for reporting defects to a human. Treats severity as a claim needing evidence: before a finding is ranked, it must survive a check that could have killed it, with reachability recorded as actual config values rather than "behind a flag". Output is two-depth — a headline per finding, with `Fires` / `Breaks` / `Checked` as one-line fields under it. Adds no classification vocabulary of its own; the mechanism line is Verified by its citation and the `Breaks` line is the one that needs a V/I/G label. User-only (`disable-model-invocation: true`) — it assumes runtime semantics, and auto-firing on a docs or config diff produces reachability ceremony for a question that does not apply.
